...
void main() {
  ...
  group('homeWidgetTest', () {
    ...
    testWidgets('HomeWidget displays lists with items', (tester) async {
      ...
      expect(
        find.textContaining(DateTime(2027).toLocal().toString().split('.')[0]),
        findsOneWidget,
      );
      ...
      expect(
        find.textContaining(DateTime(2026).toLocal().toString().split('.')[0]),
        findsOneWidget,
      );
      expect(tester.widget<Checkbox>(find.byType(Checkbox).last).value, true);
    });

    testWidgets('delete task todo triggers removeTask and shows snackbar', (
      tester,
    ) async {
      final task = Task(
        id: 1,
    ...
    testWidgets(
      'delete task complited triggers removeTask and shows snackbar',
      (tester) async {
        final task = Task(
          id: 1,
          title: 'title2',
