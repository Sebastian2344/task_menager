...
void main() {
  ...
  testWidgets('CreateTask shows all required fields and buttons',
    ...
    expect(find.widgetWithText(TextFormField, 'Nazwa zadania'), findsOneWidget);
    expect(find.widgetWithText(TextFormField, 'Opis (opcjonalny)'), findsOneWidget);
  ...
  testWidgets('Saving valid form shows snackbar and calls providers', (tester) async {
  when(() => mockTaskProvider.addTask(any(), any(), any()))
      .thenAnswer((_) async {});
  ...
  await tester.enterText(find.widgetWithText(TextFormField, 'Nazwa zadania'), 'Test task');
  await tester.enterText(find.widgetWithText(TextFormField, 'Opis (opcjonalny)'), 'Opis testowy');
